...
import { DocumentBriefcaseRegular } from "@fluentui/react-icons";
...
function CaseCounterComponent(props: IInputs) {
  ...
  const caseStatus = props.caseStatus.raw;
  const priorityCode = props.priorityCode.raw;
  ...
  React.useEffect(() => {
    ...
    // Xrm.WebApi.retrieveMultipleRecords("incident", query)
    //   .then(({ entities }) => {
  ...
  return (
    <Badge
      icon={<DocumentBriefcaseRegular fontSize={30} color={fontColor} />}
      size="extra-large"
      shape="rounded"
      style={{
      ...
      <Text as="h2">
        <Text size={400} weight="bold" style={{ color: primaryColor }}>
          {totalCases} {PriorityCode[priorityCode]}
        </Text>
        <Text size={100} block={true} style={{ color: fontColor }}>
          Priority {CaseStatus[caseStatus]} Cases
        </Text>
      </Text>
    </Badge>
  );
